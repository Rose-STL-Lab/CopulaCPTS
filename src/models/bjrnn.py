...
class bj_rnn:
    def __init__(
        self, model, mode="stochastic", damp=1e-4, rnn_mode="RNN", recursion_depth=20
    ):

        self.model = model
        self.rnn_mode = rnn_mode
        self.IF = influence_function(
            model,
            train_index=list(range(model.X.shape[0])[:200]),
            mode=mode,
            damp=damp,
        ...
        for k in range(len(self.IF)):
            ...
            self.LOBO_residuals.append(
                np.abs(
                    np.array(self.model.y[k].unsqueeze(0))
                    - np.array(perturbed_models.predict(X_[k].unsqueeze(0)).detach())
                )
            )
    ...
    def predict(self, X_test, epsilon=0.1):
        ...
        for i in range(self.dim):
            y_u_approx[:, :, i] = np.quantile(
                variable_preds[..., i]
                + np.repeat(
                    np.expand_dims(self.LOBO_residuals[..., i], axis=1),
                    num_sequences,
                    axis=1,
                )
                * 5,
        ...
        self.results_dict[epsilon] = {
            "y_pred": y_pred,
            "y_l_approx": y_l_approx,
            "y_u_approx": y_u_approx,
        }
