...
class CFRNN:
    ...
    def calibrate(self):
        ...
        nonconformity = (
            torch.norm((pred_y[..., :dim] - self.cali_y), p=2, dim=-1).detach().numpy()
        )
    ...
    def calibrate_l1(self):
        ...
        pred_y = self.model.predict(self.cali_x)
        nonconformity = (
            torch.norm((pred_y[..., :dim] - self.cali_y), p=1, dim=-1).detach().numpy()
        )
        self.nonconformity = nonconformity
    ...
    def predict(self, X_test, epsilon=0.1):
        ...
        new_quantile = min(
            (n_calibration + 1.0)
            * (1 - (epsilon / self.cali_y.shape[-2]))
            / n_calibration,
            1,
        )

        radius = [
            np.quantile(nonconformity[:, r], new_quantile)
    ...
    def calc_coverage(self, radius, y_pred, y_test):
        dim = y_test.shape[-1]
        testnonconformity = (
            torch.norm((y_pred[..., :dim] - y_test), p=2, dim=-1).detach().numpy()
        )
    ...
    def calc_coverage_l1(self, radius, y_pred, y_test):
        dim = y_test.shape[-1]
        testnonconformity = (
            torch.norm((y_pred[..., :dim] - y_test), p=1, dim=-1).detach().numpy()
        )
